...
export default function DashboardPage() {
  ...
  useEffect(() => {
    supabase.auth.getUser().then(({ data }) => {
      ...
      // Extrair nome do user_metadata (guardado no register)
      setCurrentUserName(data.user?.user_metadata?.full_name ?? null)
    })
  }, [])

  // ── Fetch confirmações do user ─────────────────────────────────────────────
  useEffect(() => {
  ...
  const handleCreate = async () => {
    ...
    if (error) {
      console.error('Supabase create FULL error:', JSON.stringify(error, null, 2))
      alert('Erro real: ' + (error?.message || 'unknown'))
      setLoading(false)
      return
    }
  ...
  // ── Confirmar (com bloqueio de duplo clique) ───────────────────────────────
  const handleConfirm = async (id: string) => {
    ...
    // Verificar na DB se já confirmou (funciona mesmo após refresh)
    ...
    if (existing) {
      // Sincronizar estado local se ainda não estava marcado
      setUserConfirmations(prev => prev.includes(id) ? prev : [...prev, id])
      return
    }

    // Inserir confirmação na tabela dedicada
    ...
    // Incrementar contador no problema
  ...
  const handleUseMyLocation = () => {
    if (!navigator.geolocation) { alert('Geolocalização não suportada neste browser.'); return }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {/* Logo — usa imagem se existir, fallback para quadrado azul */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          ...
          <div>
            <div style={{ fontFamily: mono, fontSize: 17, fontWeight: 500, letterSpacing: '0.03em', lineHeight: 1.1, color: '#111827' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginLeft: 'auto' }}>
          ...
          <button onClick={() => setShowStats(true)} style={{
            fontFamily: body, fontSize: 15, padding: '6px 12px',
          ...
          <div style={{
            ...
            padding: '4px 12px', border: '1px solid #A7F3D0',
          }}>
            <span style={{
      ...
      <div style={{ display: 'flex', flex: 1, minHeight: 0, position: 'relative' }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            {[
              ...
              <div key={s.label} style={{ background: '#ffffff', padding: '15px 14px 13px' }}>
                <div style={{ fontSize: 10, fontWeight: 500, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#9ca3af', marginBottom: 6, fontFamily: mono }}>
                  {s.label}
                </div>
                <div style={{ fontFamily: mono, fontSize: 24, fontWeight: 500, color: s.color ?? '#111827', lineHeight: 1 }}>
                  {s.value}
                </div>
              ...
              {([
                ...
                return (
                  <button key={f} onClick={() => setActiveFilter(f)} style={{
                    fontFamily: body, fontSize: 14, padding: '5px 14px', borderRadius: 20,
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: 10, display: 'flex', flexDirection: 'column', gap: 7 }}>
            {filtered.length === 0 && (
              <div style={{ textAlign: 'center', padding: 40, fontSize: 14, color: '#9ca3af', fontFamily: mono }}>
                {t.noResults}
              </div>
            )}
            {filtered.map((p, idx) => {
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    borderRadius: 11, padding: '14px 14px', cursor: 'pointer',
                    animation: `slideIn 0.2s ease ${idx * 0.025}s both`,
                  }}
                >
                  {/* Header */}
                  <div style={{ display: 'flex', alignItems: 'flex-start', gap: 9, marginBottom: 7 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 16, fontWeight: 500, lineHeight: 1.3, color: '#111827', marginBottom: (isOwner || authorLabel) ? 4 : 0 }}>
                        {p.name}
                      </div>
                      <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap' }}>
                        {isOwner && (
                          <span style={{
                            fontFamily: mono, fontSize: 10, color: '#1A56DB',
                            background: '#EFF6FF', borderRadius: 4,
                            padding: '1px 6px', border: '1px solid #BFDBFE',
                          }}>
                            {t.mine}
                          </span>
                        )}
                  ...
                  {/* Desc */}
                  <div style={{ fontSize: 14, color: '#6b7280', lineHeight: 1.6, marginBottom: 10, paddingLeft: 18 }}>
                    {p.description}
                  </div>

                  {/* Meta */}
                  <div style={{ display: 'flex', alignItems: 'center', paddingLeft: 18, marginBottom: 10 }}>
                    <span style={{ fontFamily: mono, fontSize: 12, color: '#9ca3af', display: 'flex', alignItems: 'center', gap: 3 }}>
                    ...
                    <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 5 }}>
                      ...
                      <span style={{ fontFamily: mono, fontSize: 11, color: '#9ca3af' }}>{vib}</span>
                    </div>
                  </div>

                  {/* Actions */}
                  <div style={{
                    display: 'flex', gap: 5, paddingLeft: 18,
                    paddingTop: 10, borderTop: '1px solid #f3f4f6',
                    alignItems: 'center', flexWrap: 'wrap',
                  }}>
                    {/* Botão Confirmar / Confirmado */}
                    <button
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          ...
          <div style={{
            ...
            padding: '8px 18px', background: '#ffffff',
            borderBottom: '1px solid #e5e7eb', flexShrink: 0,
          }}>
            <div style={{ fontFamily: mono, fontSize: 13, color: '#9ca3af' }}>
            ...
            <div style={{
              ...
              borderRadius: 7, padding: '5px 13px', transition: 'all 0.2s',
            }}>
              {formMode
          ...
          {/* Detalhe */}
          <div style={{
            height: 195, background: '#ffffff',
            borderTop: '1px solid #e5e7eb',
            padding: '18px 28px', flexShrink: 0, overflowY: 'auto',
          }}>
            {!selected ? (
              <div style={{
                display: 'flex', alignItems: 'center', justifyContent: 'center',
                height: '100%', fontSize: 15, color: '#9ca3af', fontFamily: mono,
              }}>
                {t.detailEmpty}
              </div>
            ) : (
              <div style={{ animation: 'fadeUp 0.2s ease' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 9, marginBottom: 6 }}>
                  <SevDot g={selected.gravidade} size={11} />
                  <div style={{ fontSize: 18, fontWeight: 500, color: '#111827' }}>{selected.name}</div>
                </div>
                <div style={{ fontSize: 14, color: '#6b7280', marginBottom: 16, lineHeight: 1.6 }}>
                  {selected.description}
                </div>
                <div style={{ display: 'flex', gap: 32, flexWrap: 'wrap', rowGap: 12 }}>
                  {[
                    ...
                    <div key={f.label}>
                      <div style={{ fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#9ca3af', fontFamily: mono, marginBottom: 4 }}>
                        {f.label}
                      </div>
                      <div style={{ fontSize: 16, fontWeight: 400, color: '#111827' }}>{f.value}</div>
                    </div>
                  ))}
                  <div>
                    <div style={{ fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#9ca3af', fontFamily: mono, marginBottom: 6 }}>
                      {t.detailScore}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <div style={{ width: 88, height: 5, background: '#e5e7eb', borderRadius: 3, overflow: 'hidden' }}>
                        <div style={{ width: `${getVibrancy(selected)}%`, height: '100%', background: '#1A56DB', borderRadius: 3 }} />
                      </div>
                      <span style={{ fontSize: 15, fontFamily: mono, color: '#111827' }}>{getVibrancy(selected)}/100</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <button onClick={closeForm} style={{
                ...
                color: '#9ca3af', fontSize: 24, lineHeight: 1, padding: '0 4px',
              }}>×</button>
            </div>
              ...
              <div style={{
                ...
                {previewPin ? (
                  <>
                    ...
                    <button
                      onClick={() => { setPreviewPin(null); setForm(prev => ({ ...prev, lat: null, lng: null, location: '' })) }}
                      style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#059669', fontSize: 20, lineHeight: 1 }}
                    >×</button>
                  </>
                ) : (
                  <span>{t.fLocPh}</span>
            ...
            <div style={{ display: 'flex', gap: 8, marginTop: 2 }}>
              ...
              <button
                ...
                style={{
                  flex: 2, padding: '11px 0', fontFamily: body,
                  fontSize: 15, fontWeight: 500, border: 'none', borderRadius: 9,
      ...
      {showStats && (
        ...
        >
          <div style={{
            ...
            border: '1px solid #e5e7eb',
            padding: 34, width: 530, maxWidth: '90vw',
            animation: 'fadeUp 0.2s ease',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 28 }}>
              <div style={{ fontSize: 18, fontWeight: 500, color: '#111827' }}>{t.statsTitle}</div>
              <button onClick={() => setShowStats(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9ca3af', fontSize: 26, lineHeight: 1 }}>×</button>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 28 }}>
              {[
                ...
                <div key={s.label} style={{ background: s.bg, borderRadius: 12, padding: '18px 20px', border: `1px solid ${s.color}22` }}>
                  <div style={{ fontSize: 11, fontFamily: mono, letterSpacing: '0.07em', textTransform: 'uppercase', color: s.color, marginBottom: 9, opacity: 0.8 }}>
                    {s.label}
                  </div>
                  <div style={{ fontFamily: mono, fontSize: 32, fontWeight: 500, color: s.color, lineHeight: 1 }}>
                    {s.value}
                  </div>
                  <div style={{ fontSize: 13, color: s.color, marginTop: 5, opacity: 0.7 }}>{s.sub}</div>
                </div>
              ))}
            </div>

            <div style={{ marginBottom: 12 }}>
              <div style={{ fontSize: 11, fontFamily: mono, color: '#9ca3af', marginBottom: 14, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
                {t.sevDist}
              </div>
              {([
                [3, t.sevHigh, '#DC2626'],
                ...
                return (
                  <div key={g} style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                    <div style={{ width: 50, fontSize: 14, color: '#6b7280', fontFamily: body, flexShrink: 0 }}>{label}</div>
                    <div style={{ flex: 1, height: 8, background: '#f3f4f6', borderRadius: 4, overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', background: color, borderRadius: 4, transition: 'width 0.4s ease' }} />
                    </div>
                    <div style={{ width: 28, fontSize: 14, fontFamily: mono, color: '#9ca3af', flexShrink: 0 }}>{count}</div>
                  </div>
                )
              })}
            </div>
